...
// Practical Example
val df2 = df.withColumn("Year",year(df("Date"))) // not just a column name is passed by the column itself

// Mean per Year, notice large 2008 drop!
val dfavgs = df2.groupBy("Year").mean()
dfavgs.select($"Year",$"avg(Close)").show()
// That shows average (can do the same with min and max) for every year
